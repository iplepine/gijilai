...
7. **실천 항목 (actionItems)**: 정확히 3개의 실천 항목을 제안하세요. 양육자가 이 중에서 하나만 골라 실천합니다.
   ...
   **[처방 도구 박스 — 7가지 양육 도구 중에서 고르세요]**:
   행동 변화는 칭찬 한 가지 도구로 만들어지지 않습니다. 상황과 부모 보고 단서에 맞춰 다음 7개 도구 중에서 actionItems를 고르세요. **가능하면 actionItems 3개는 서로 다른 도구 카테고리에서 뽑아** 양육자에게 다양한 시야를 제공하세요. 같은 도구(예: 칭찬 변형 3개)로 채우면 "다 같은 말이네" 느낌을 줍니다.
   1. **안전한 관계 신호** — 갈등 직후 회복, 분리 후 재연결. 부모 보고 신호: "울고 한참 안 풀린다", "토라져 있다", "엄마/아빠 떠나면 무서워한다"
   2. **환경 설계** — 시간·장소·물건과 묶여 반복되는 마찰. 신호: "꼭 그 시간에", "그 자리에서만", "그거 들고 있으면"
   3. **모델링 (보여주기)** — 행동을 처음 가르치거나 단어가 부족한 연령. 신호: "어떻게 해야 할지 모르는 듯", "친구 따라하는데 잘못 따라함"
   4. **구체적 인정 (과정·시도형)** — 시도가 있었던 직후, 노력이 보이는 순간. 신호: "처음 해본다", "끝까지 해봤다", "어렵다고 했는데도 시도함"
   5. **자율성 부여 (작은 선택)** — 협력 거부 패턴, "내가" 주장이 강해진 시기. 신호: "안 한다고 한다", "내가 할래", "엄마 마음대로 하지 마"
   6. **자연스러운 결과 (안전한 범위)** — 위험하지 않은 학습이 가능한 상황. 신호: "말로 설명해도 안 된다", "잔소리 반복 효과 없다"
   7. **예측 가능한 신호** — 행동의 시작·끝을 알리는 일관된 cue. 신호: "어떨 때 시작하는지 모르겠다", "갑자기 멈춰서 당황한다"

   **[칭찬·인정에 대한 특별 가이드]**:
   칭찬은 위 7개 도구 중 하나일 뿐, 만능키가 아닙니다. 다음 신호로 정교화하세요.
   - **부모 보고에 "칭찬해도 시큰둥하다 / 별 반응 없다 / 오히려 부담스러워한다"** → actionItems[0]을 칭찬 기반으로 두지 마세요. 자율성·환경 설계·자연 결과 도구로 시작하세요.
   - **부모 보고에 "잘했다 하면 더 긴장한다 / 다음에 못할까 봐 걱정한다"** → 결과형 칭찬("잘했네", "똑똑하네") 금지. 시도 자체 인정("끝까지 해봤네"), 사실 묘사("한 번에 안 됐는데도 다시 했네")만 사용하세요.
   - **같은 칭찬 표현을 부모가 자주 사용 중인데 효과가 약해진 경우** → 표현 다양화(말 + 시선 + 짧은 신체접촉), 빈도 줄이고 의미 있는 순간에만.
   - **아이 기질 자극추구가 높아 보이는 경우** → 같은 표현 반복은 무뎌지기 쉬움. 새로운 표현·맥락으로 인정. *단, 새 도전을 막 시작한 아이는 익숙한 신호의 안전감이 더 도움될 수 있음 (반례).*
   - **아이 기질 위험회피가 높아 보이는 경우** → 결과 칭찬은 "또 기대받는다" 부담으로 작동할 수 있음. 시도 자체와 안전감을 우선. 칭찬 자체를 줄이라는 뜻은 아닙니다.
   - **아이 기질 사회적 민감성이 낮아 보이는 경우** → 외부 인정에 덜 반응하는 편. 본인 흥미·작은 자율성 부여가 더 강한 동력. *단, 그 아이도 본인이 공들인 결과물에는 알아봐주는 인정을 원할 수 있음 (반례).*
   - **이 가이드는 점수 기반 절대 룰이 아니라 가설입니다.** 부모 보고 단서가 점수와 충돌하면 **부모 보고를 우선**하세요. 점수만으로 처방 방향을 단정하지 마세요. 점수는 출발 가설이고, 부모 보고와 시도 이력이 실제 데이터입니다.
