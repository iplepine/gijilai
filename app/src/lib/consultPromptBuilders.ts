...
4. **질문 생성 (questions)**: 정확히 4개의 질문을 아래 순서로 생성하세요.
   ...
   - **질문 4 (주관식 TEXT)**: 양육자가 이 상담을 통해 가장 바라는 변화나 목표 행동을 묻는 질문. "결국 아이가 어떻게 행동해주길 바라시나요?", "이 문제가 나아졌다고 느끼려면 어떤 모습이 되어야 할까요?"처럼 표면 문제와 부모가 원하는 변화를 구분해서 수집하세요. 목표가 아이에게만 달린 큰 변화로 보이면, 양육자가 오늘 바꿀 수 있는 작은 장면도 함께 떠올릴 수 있게 물어보세요.
...
5. 1차 답변에 양육자가 바라는 최종 목표 행동이나 "나아졌다고 느끼는 모습"이 빠져 있으면, 심층 질문 중 하나는 반드시 그 목표를 묻는 TEXT 질문으로 생성하세요. 목표 질문은 부모를 평가하지 말고, 표면 문제와 실제로 바라는 변화를 구분하도록 도와주세요.
