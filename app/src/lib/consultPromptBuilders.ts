...
4. **문진 해설 (questionAnalysis)**: 각 문진 질문과 양육자의 답변을 해설하세요. 이 섹션의 목적은 기질 라벨을 붙이는 것이 아니라, **이 처방이 왜 나왔는지 양육자가 납득하도록 근거를 보여주는 것**입니다. 각 항목은 질문 원문(question), 답변 원문(answer), 해설(analysis)로 구성합니다.
...
7. **실천 기록 연계**: 이전 상담의 실천 기록을 참고하여, 효과적이었던 방법은 강화하고 효과 없었던 것은 다른 접근을 제안하세요. "지난번에 ~를 시도하셨는데"와 같이 자연스럽게 언급하세요. 현재 고민에 "여전히", "다시", "계속"처럼 반복 신호가 있으면 이전 실천 항목을 그대로 반복하지 말고, 같은 목표를 더 작은 행동·다른 타이밍·더 명확한 경계 중 하나로 조정하세요.` : params.recentObservations && params.recentObservations.length > 0 ? `
...
${!isFollowUp ? `8. **세션 제목 (sessionTitle)**: 이 고민을 한 줄(15자 이내)로 요약한 제목을 생성하세요. 진단명처럼 보이는 표현보다 생활 장면 중심 제목을 사용하세요. 예: "등원 현관 울음", "형제 장난감 싸움", "밥 안 먹는 문제"` : ''}
