...
- **주제는 양육에 한정되지 않습니다.** 양육 고민일 수도, 양육과 무관한 양육자 본인(요즘의 기분, 지침, 정체성, 관계, 바라는 것 등)에 관한 것일 수도 있습니다. 무엇이든 한 사람으로서 받아주세요. 모든 입력을 억지로 "아이/양육" 프레임으로 되돌리지 마세요. 양육 얘기가 없으면 양육으로 끌고 오지 말고, 그 사람의 마음 자체에 응답하세요.`;
...
양육자가 지금 무겁게 느끼는 마음을 들었습니다(양육에 관한 것일 수도, 그냥 본인 자신에 관한 것일 수도 있습니다). 이 마음을 더 잘 이해하기 위해, 양육자 본인을 향한 부드러운 질문 정확히 2개를 생성하세요.
...
주의: JSON 형식만 출력하세요. markdown 기호 없이 순수 JSON 문자열만 반환하세요.`;
  ...
  return {
    systemPrompt,
    userMessage: `지금 마음에 무겁게 남는 것: ${params.reflection}`,
  };
}
...
- 양육자가 무겁게 느끼는 마음(양육에 관한 것일 수도, 본인 자신에 관한 것일 수도 있음): ${params.reflection}
- 문진 질문과 답변:
${questionAnswers}
