...
class KakaoFlutterSdkPlugin : MethodCallHandler, FlutterPlugin, ActivityAware,
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            "accountLogin" -> {
                ...
                val intent = IntentFactory.customTabsForLogin(activity, args, resultReceiver(result))
            ...
            "launchBrowserTab" -> {
                ...
                val args = call.arguments as Map<String, String?>
                val intent =
                    IntentFactory.customTabs<CustomTabsActivity>(activity, args, resultReceiver(result))
                activity.startActivity(intent)
            }
            ...
            "authorizeWithTalk" -> {
                try {
                    ...
                    val intent =
                        IntentFactory.talkAuthCode(activity, args, resultReceiver(result))
                    activity.startActivity(intent)
                } catch (e: Exception) {
            ...
            "selectShippingAddress" -> {
                ...
                val intent =
                    IntentFactory.customTabs<AppsHandlerActivity>(activity, args, resultReceiver(result))
                activity.startActivity(intent)
            }
    ...
    private fun resultReceiver(pendingResult: Result): SingleResultReceiver<String> {
        var isCompleted = false
        return SingleResultReceiver.create(
            emitter = { url, error ->
                if (!isCompleted) {
                    isCompleted = true
                    if (error != null) {
                        resultCanceled(pendingResult, error.first, error.second)
                    } else {
                        resultOk(pendingResult, url)
                    }
                }
            },
