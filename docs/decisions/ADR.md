...
## 2026-05-05 | 실천 리마인더 알림은 quick check-in 입력을 지원한다

- **결정**: 앱 로컬 실천 리마인더 알림에 `했어요`, `못했어요`, `열기` 액션을 제공한다. `했어요`와 `못했어요`는 알림 답장 입력을 받아 대표 실천의 오늘 로그로 저장하고, 메모 원문은 URL 쿼리가 아니라 네이티브 WebView 이벤트로 전달한다.
- **이유**: 리마인더의 목적은 앱 재방문 자체보다 `상담 → 실천 → 기록 → 다음 상담` 루프의 기록 마찰을 줄이는 것이다. 한줄 메모는 다음 상담 맥락에 충분히 도움이 되지만 상담 고민 입력처럼 길고 민감한 원문은 알림에서 받기에 부적절하다.
- **대안**: 알림 탭 시 실천 화면만 열기 — 기존보다 입력 마찰이 남아 알림의 행동 전환 효과가 약하다. 알림에서 상담 고민이나 상세 아이 반응까지 받기 — 검증과 개인정보 범위가 커지고 입력 품질이 떨어져 기각.
