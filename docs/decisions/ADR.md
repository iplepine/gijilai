...
## 2026-05-05 | 앱 로그인 화면은 Flutter 네이티브 UI를 기본 경로로 둔다

- **결정**: 앱 WebView에서 비로그인 사용자는 웹 랜딩/웹 로그인 화면을 보지 않고 Flutter 네이티브 로그인 화면으로 진입한다. 이메일 로그인/회원가입도 네이티브 폼에서 처리하고 `/auth/native-email-session`으로 Supabase 세션 쿠키를 설정한다. `/login` 웹 라우트와 `AuthBridge` OAuth handoff는 네이티브 토큰을 받을 수 없는 경우의 fallback으로 유지한다.
- **이유**: 앱 설치 후 첫 로그인에서 웹 페이지를 거쳐 보이면 앱다운 신뢰감이 떨어지고, 이메일 로그인만 웹 폼으로 빠지는 경험도 일관성이 낮다. Flutter 네이티브 화면을 기본 경로로 두면 소셜/이메일 로그인 모두 같은 앱 컨텍스트에서 시작하면서 기존 Supabase 쿠키 기반 WebView 세션 구조는 유지할 수 있다.
- **대안**: 완전 네이티브 Supabase 세션 저장으로 전환 — 이후 모든 API 호출과 WebView 쿠키 동기화 범위가 커져 현재 WebView 중심 아키텍처에 비해 변경 위험이 크다. 웹 로그인 UI 유지 — 구현은 단순하지만 앱 첫 진입 경험 개선 목표를 충족하지 못한다.
