...
- **결정**: 설문 안내 화면(`/survey/intro`)은 단일 시작 버튼 대신 두 경로를 명시적으로 제공한다. `3분 아이 기질 검사`는 아이 기질 설문 완료 후 즉시 `/report?child_only=true`로 이동하고, `처음부터 전체 분석 시작하기`는 아이 설문 뒤 곧바로 양육자 기질/양육 스타일 설문까지 연속 진행한다.
