...
최근 사용성 점검에서 하단 탭 아이콘 잘림, 홈 스크롤 reveal 애니메이션, 다자녀 맥락 구분, "실천 정리하기" 용어, 홈 대표 실천 개수 표시, 실천 마무리 회고 필수 입력, 음성 입력 첫 탭이 키보드 포커스로 소비되는 문제가 마찰로 확인됐다.
...
- [x] 음성 버튼이 첫 탭에서 textarea 포커스 대신 바로 음성 입력 시작을 시도
...
- 상담/실천 textarea의 음성 버튼을 키보드가 내려간 상태에서 첫 탭해 바로 음성 입력이 열리는지 확인
