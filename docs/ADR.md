...
- **결정**: 설문 안내 화면(`/survey/intro`)은 단일 시작 버튼 대신 두 경로를 명시적으로 제공한다. `빠르게 아이 결과 먼저 보기`는 아이 기질 설문 완료 후 즉시 `/report?child_only=true`로 이동하고, `처음부터 전체 분석 시작하기`는 아이 설문 뒤 곧바로 양육자 기질/양육 스타일 설문까지 연속 진행한다.
...
- **결정**: `/report` 개선은 감이나 단일 페이지 조회수가 아니라 Firebase/GA4 이벤트 퍼널 기준으로 운영한다. 기본 퍼널은 `report_viewed → report_primary_cta_clicked → pricing_viewed → payment_started → payment_completed`로 두고, `child_only` 확장 흐름은 별도 퍼널로 관리한다.
