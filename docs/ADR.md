...
## 2026-04-16 | 실천 리마인더는 로컬 설정 MVP로 먼저 제공

- **결정**: 실천 탭에 변화 요약 패널과 알림 설정 진입점을 추가하고, 알림 설정 페이지에서 실천 리마인더 사용 여부와 시간을 브라우저 `localStorage`에 저장한다.
- **이유**: 실천 반복 사용률을 높이는 UX를 빠르게 검증해야 한다. 실제 푸시 발송은 앱 푸시 토큰 저장, 서버 스케줄러, 수신 권한/동의 관리가 필요해 독립 작업으로 분리하는 편이 안전하다.
- **대안**: 서버 푸시 인프라까지 한 번에 구현 — DB 스키마, 워커, 앱 토큰 연동, 플랫폼별 권한 처리가 함께 필요해 이번 범위에서는 과도하므로 보류.
