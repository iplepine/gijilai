...
## 2026-04-16 | Google 로그인은 커스텀 Auth 도메인 전까지 보조 옵션으로 노출

- **결정**: Supabase Auth 커스텀 도메인을 도입하기 전까지 로그인 화면에서 카카오를 기본 CTA로 유지하고, 이메일을 보조 버튼으로 노출하며, Google 로그인은 작은 보조 옵션으로 낮춘다.
- **이유**: 현재 Google OAuth 화면에서 Supabase 프로젝트 도메인이 표시되어 사용자가 낯선 외부 도메인으로 이동한다고 느낄 수 있다. 커스텀 도메인은 Supabase 유료 기능이므로, 비용을 쓰기 전까지 신뢰도가 높은 국내 카카오/이메일 흐름을 우선한다.
- **대안**: (1) Supabase Pro/custom domain 결제 — 가장 깔끔하지만 즉시 비용 발생 (2) 자체 Google OAuth 구현 — 인증 보안과 Supabase 세션 연동 복잡도가 커서 기각
