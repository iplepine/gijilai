...
- **결정**: 구독이 해지 예약(`cancelled_at` 존재) 상태일 때 구독 관리(`/settings/subscription`)와 요금제(`/pricing`) 화면 모두에서 재개 동선을 버튼으로 노출한다. `PORTONE`은 앱 내 `구독 계속하기` 버튼으로 `/api/payment/reactivate-subscription`을 호출하고, `APPLE_IAP`/`GOOGLE_PLAY`는 `스토어 열기` 버튼으로 각 스토어 구독 관리 화면으로 이동시킨다.
