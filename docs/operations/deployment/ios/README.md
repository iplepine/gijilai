...
- iOS 시뮬레이터 `Debug` 실행에서 결제 UX만 확인할 때는 `--dart-define=GIJILAI_ENABLE_IOS_IAP_FALLBACK=true`를 넘긴다. 이 모드에서는 앱이 `gijilai_app/ios/Runner/Configuration.storekit`으로 로컬 StoreKit 테스트를 시도하고, Flutter `in_app_purchase` 조합에서 시뮬레이터 상품 조회가 비면 네이티브 테스트 다이얼로그(`성공/실패/취소`)로 결제 로딩/결과 플로우를 계속 확인할 수 있다.
...
- 실제 샌드박스/스토어 상품 테스트를 실기기에서 할 때는 기본 `Runner`를 실기기 대상으로 실행한다. App Store Connect에서 첫 구독 상품이 `심사 대기 중`이면 실제 스토어 상품 조회와 결제 검증은 통과하지 않을 수 있다. 시뮬레이터 fallback은 앱 내부 UX 검증용이며, 실제 영수증 검증과 구독 상태 동기화는 구독 상품이 앱 버전과 함께 승인된 뒤 실기기 샌드박스로 별도 확인해야 한다.
