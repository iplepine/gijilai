...
웹 앱의 디렉토리 책임, 의존 방향, 상태/모듈 분리 원칙은 [WEB_ARCHITECTURE.md](WEB_ARCHITECTURE.md)를 우선 참고한다. 이 문서는 코드 스타일과 구현 규칙 중심으로 유지한다.
...
- 공통 패턴으로 해결되지 않는 화면별 예외는 해당 페이지 코드에 이유가 드러나게 남기고, 필요하면 `docs/product/policies/navigation.md`에 규칙을 추가한다.
