...
- OAuth fallback은 Kakao Developers에서 설정되지 않은 `account_email` 요청으로 `KOE205`가 나지 않도록 `profile_nickname` scope만 명시한다. 이메일을 사용하려면 Kakao Developers 동의항목에서 `account_email`을 활성화한 뒤 scope 정책을 다시 조정한다.
