...
- 브라우저에서 구독/결제 진입(`/pricing`, `/payment`) 시에는 웹 결제를 계속 진행하지 않고 앱 설치 랜딩(`/install-app`)으로 보낸다. 랜딩 페이지는 접속 브라우저 기준으로 iOS는 App Store, Android는 Google Play 단일 CTA를 노출하고, 데스크톱/기타 브라우저에서만 App Store와 Google Play 선택지를 함께 보여준다.
...
> 참고:
> `checkCooldown` (`app/src/lib/dateUtils.ts`)은 구독 여부와 무관하게 24시간 고정이다. 현재 요금제/구독 유도 문구는 쿨다운 면제보다 상담-실천-후속상담의 연속 관리 가치를 우선 설명한다.
